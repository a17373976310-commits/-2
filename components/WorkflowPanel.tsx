...
export const WorkflowPanel: React.FC<WorkflowPanelProps> = ({
  ...
  useEffect(() => {
    // Load workflows from persistent storage with migration
    const loadWorkflows = async () => {
      try {
        // Try to load from server first
        const serverWorkflows = await historyService.getWorkflows();
        console.log('[WorkflowPanel] 从服务器加载:', serverWorkflows.length, '个工作流');

        // Check localStorage for any existing workflows that need migration
        const localData = localStorage.getItem('gemini_workflows');
        const localWorkflows = localData ? JSON.parse(localData) : [];

        if (serverWorkflows.length === 0 && localWorkflows.length > 0) {
          // Migrate localStorage workflows to server
          console.log('[WorkflowPanel] 迁移 localStorage 数据:', localWorkflows.length, '个工作流');
          await historyService.saveWorkflows(localWorkflows);
          setWorkflows(localWorkflows);
          logger.success(`已迁移 ${localWorkflows.length} 个工作流到服务器`);
        } else if (serverWorkflows.length > 0) {
  ...
  const saveWorkflows = async (list: Workflow[]) => {
    setWorkflows(list);
    // Save to both server and localStorage
    await historyService.saveWorkflows(list);
